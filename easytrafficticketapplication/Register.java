...
public class Register extends AppCompatActivity {
    ...
    public void rclick(View v){
        ...
        if(usn.toString().trim().length() <= 0 ||pass.toString().trim().length() <= 0 ||pass2.toString().trim().length() <= 0){
            Toast.makeText(Register.this, "Fill all fields to continue", Toast.LENGTH_LONG).show();
        }else{
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, LOGIN_URL,
                ...
                new Response.ErrorListener() {
                    ...
                    public void onErrorResponse(VolleyError error) {
                        Toast.makeText(Register.this, "This registration number is already registered for this application", Toast.LENGTH_LONG).show();
                    }
                }) {
            @Override
